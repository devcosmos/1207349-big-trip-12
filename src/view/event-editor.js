...
const createEventTypeTemplate = (type, count) => {
  return (
...
const createEventDestinationTemplate = (eventType, cities, currentDestination, count) => {
  ...
  return (
    `<div class="event__field-group  event__field-group--destination">
      ...
      <input 
        ...
        id="event-destination-${count}" 
        type="text" 
        name="event-destination" 
        value="${currentDestination === null ? `` : currentDestination}" 
        list="destination-list-${count}">
...
const createEventOffersTemplate = (acceptedOffers, eventType, count) => {
...
const createEventEditorTemplate = (event, count, cities) => {
  const {eventType, currentDestination, acceptedOffers, description, dateStart, dateEnd, cost} = event;

  const eventTypeTemplate = createEventTypeTemplate(eventType, count);
  const eventDestinationTemplate = createEventDestinationTemplate(eventType, cities, currentDestination, count);
  const eventOffersTemplate = createEventOffersTemplate(acceptedOffers, eventType, count);
    ...
    `<form class="trip-events__item  event  event--edit" action="#" method="post">
      <header class="event__header">
        ...
        <div class="event__field-group  event__field-group--time">
          ...
          <input class="event__input  event__input--time" id="event-start-time-${count}" type="text" name="event-start-time" value="${dateStartEventAtFormat}">
          &mdash;
          <label class="visually-hidden" for="event-end-time-${count}">
            To
          </label>
          <input class="event__input  event__input--time" id="event-end-time-${count}" type="text" name="event-end-time" value="${dateEndEventAtFormat}">
        </div>
        ...
        <div class="event__field-group  event__field-group--price">
          ...
          <input class="event__input  event__input--price" id="event-price-${count}" type="text" name="event-price" value="${cost === null ? `` : cost}">
        </div>
...
export default class EventEditor {
  constructor(event = BLANK_EVENT, eventCount, cities) {
    this._element = null;
    this._event = event;
    this._cities = cities;
    this._count = eventCount;
  }

  getTemplate() {
    return createEventEditorTemplate(this._event, this._count, this._cities);
  }
